...
print("loading data...")
preprocess_name = 'preprocessing_BikeDC.pkl'
...
X_train_all = rearrange(X_train_all, "n (c1 c) h w -> n c h w c1",c1=2)
X_test = X_test[0]
X_test = rearrange(X_test, "n (c1 c) h w -> n c h w c1",c1=2)
Y_train_all = np.transpose(Y_train_all,(0,2,3,1)) 
...
def train_model(lr, batch_size, num_hidden, encoder_length, decoder_length, save_results=False, i=''):
    ...
    # model.summary()
    hyperparams_name = 'BikeDC{}.c{}.p{}.t{}.num_hidden_{}.encoder_length_{}.decoder_length_{}.lr_{}.batchsize_{}'.format(
    ...
    if (save_results):
        ...
        # save to csv
        csv_name = os.path.join('results', 'pred-cnn_BikeDC_results.csv')
...
# save best params
params_fname = 'pred-cnn_BikeDC_best_params.json'
